...
# Initialize OpenRouter client (free tier)
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY", "sk-or-v1-2c5a2f9d8b3e7a1f4c6d8e0f2a4b6c8d0e2f4a6b8c0d2e4f6a8b0c2d4e6f8a0b2")
OPENROUTER_BASE_URL = "https://openrouter.ai/api/v1"

# Free models available on OpenRouter (choose one)
FREE_MODELS = [
...
DEFAULT_MODEL = FREE_MODELS[0]  # Using Gemini 2.0 Flash as default
...
if OPENROUTER_API_KEY:
    ...
    st.sidebar.success("✅ OpenRouter API configured (Free Tier)")
else:
    client = None
    st.sidebar.warning("⚠️ OpenRouter API key not configured. Some features will be limited.")
...
with st.sidebar:
    ...
    selected_model = st.selectbox(
        ...
        FREE_MODELS,
        index=0,
        help="Choose which free AI model to use for extraction"
    )
    
    # Display model info
    model_info = {
        "google/gemini-2.0-flash-exp:free": "⚡ Fast, good for structured data extraction",
        "google/gemini-flash-1.5": "🚀 Balanced performance",
        "microsoft/phi-3-mini-128k-instruct:free": "📚 Small but capable, good for text",
        "meta-llama/llama-3.2-3b-instruct:free": "🦙 Lightweight Llama model",
        "mistralai/mistral-7b-instruct:free": "🎯 Good general purpose",
        "qwen/qwen-2.5-7b-instruct:free": "🌊 Strong multilingual support"
    }
...
"""Extract tables from PDFs using OpenRouter's free AI models."""
if not client:
    st.error("❌ OpenRouter API not configured. Please add OPENROUTER_API_KEY to .env file.")
    return []
...
try:
    ...
    # Limit pages to avoid rate limits on free tier
    max_pages = min(total_pages, 20)  # Free tier limit
    if total_pages > max_pages:
        st.warning(f"⚠️ PDF has {total_pages} pages. Free tier will process first {max_pages} pages.")
    ...
    for page_num in range(1, max_pages + 1):
        try:
            ...
            response = client.chat.completions.create(
                model=selected_model,  # Use selected free model
                max_tokens=4096,
                messages=[
                    {
                        "role": "user",
...
Return ONLY valid JSON, no additional text."""
            ...
            try:
                ...
                st.warning(f"⚠️ Could not parse AI response for page {page_num}")
                all_pages.append(pd.DataFrame())
...
with st.sidebar:
    st.markdown("### 🚀 OpenRouter Status")
    if client:
        st.success(f"✅ Connected to OpenRouter\nUsing: {selected_model.split('/')[-1]}")
        st.caption("Free tier - Rate limits apply")
    else:
...
for file in pdf_files:
    ...
    if (not camelot_pages or all(df.empty for df in camelot_pages)) and client:
        st.info(f"🤖 Trying AI extraction for {file.name} using {selected_model.split('/')[-1]}...")
        camelot_pages = extract_openai_pagewise(tmp_path)
    elif (not camelot_pages or all(df.empty for df in camelot_pages)):
        st.warning(f"⚠️ Could not extract data from {file.name}. Please check if the PDF contains machine-readable tables.")
