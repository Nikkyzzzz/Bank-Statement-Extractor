...
if uploaded_files:
    ...
    for file_idx, file in enumerate(uploaded_files):
        ...
        with st.spinner(f"Extracting data from {file.name} ..."):
            # Try OpenAI vision API first (works best for complex/borderless tables)
            if client:
                st.info("🤖 Using OpenAI Vision API for extraction...")
                ...
                if openai_pages and any(not df.empty for df in openai_pages):
                    ...
                    st.info("📊 OpenAI extraction returned no results, trying traditional methods...")
                ...
                # Traditional methods if OpenAI not available
                st.warning("⚠️ OpenAI API not configured. Using traditional extraction methods.")
                camelot_pages = extract_camelot_pagewise(tmp_path, "lattice")
