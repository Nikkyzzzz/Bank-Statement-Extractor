...
def split_merged_decimals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    SIMPLE LOGIC:
    - If Balance column has TWO decimal numbers merged together
    - First number (with 2 digits after decimal) → Previous column (Deposits/Withdrawal)
    - Second number → Stays in Balance column
    
    Example: Balance = "13,48,348.00 -3,49,48,64,718.53"
    becomes: Deposits = "13,48,348.00", Balance = "-3,49,48,64,718.53"
    """
    df_split = df.copy()
    ...
    # Matches: "number.XX space number.XX" where XX = 2 digits after decimal
    ...
    for row_idx in range(len(df_split)):
        ...
        if matches and len(matches) > 0:
            ...
            # First number → Previous column (Deposits or Withdrawal)
            if balance_col > 0:
...
def extract_page_with_strategy(path, page_num, primary_flavor="lattice"):
    ...
    try:
        ...
        if tables and len(tables) > 0:
            ...
            # Use it as the column names, and handle duplicates
            if len(extracted_df) > 0:
                header_row = extracted_df.iloc[0]
                # Clean up column headers and make them unique
                new_cols = [re.sub(r"\s+", " ", str(c)).strip() for c in header_row]
                
                # Make unique column names (add suffix if duplicates exist)
                seen = {}
                unique_cols = []
                for col in new_cols:
                    if col in seen:
                ...
                extracted_df.columns = unique_cols
            ...
            # Split any merged decimal values in cells
            extracted_df = split_merged_decimals(extracted_df)
...
if uploaded_files:
    ...
    for file_idx, file in enumerate(uploaded_files):
        ...
        file_extracted_data = []  # Collect data for this specific file
        ...
        for page_num, extracted_df in enumerate(camelot_pages, start=1):
            ...
            # Show extracted data
            if not extracted_df.empty:
                st.markdown(f"**Page {page_num}** (Camelot - {len(extracted_df)} rows)")
            ...
            if not extracted_df.empty:
                ...
                file_extracted_data.append(extracted_df)  # Track per file
