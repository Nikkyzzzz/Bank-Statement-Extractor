...
if uploaded_files:
    ...
    # Single download for ALL extracted data (zip of per-file CSVs + Excels)
    if file_data_map:
        try:
            st.markdown("---")
            ...
            with zipfile.ZipFile(zip_buffer, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
                # Add per-file CSV and Excel into zip
                for file_name, file_dfs in file_data_map.items():
                    combined_file_df = pd.concat(file_dfs, ignore_index=True)

                    # CSV
                    csv_bytes = combined_file_df.to_csv(index=False).encode("utf-8")
                    zf.writestr(f"{file_name.replace('.pdf','')}_extracted.csv", csv_bytes)

                    # Excel
                    excel_buf = io.BytesIO()
                    with pd.ExcelWriter(excel_buf, engine='openpyxl') as writer:
                        combined_file_df.to_excel(writer, index=False, sheet_name='Data')
                    excel_buf.seek(0)
                    zf.writestr(f"{file_name.replace('.pdf','')}_extracted.xlsx", excel_buf.read())

            zip_buffer.seek(0)
            st.download_button(
                label="📥 Download ALL (CSV + Excel) — ZIP",
                data=zip_buffer,
            ...
            st.error(f"Error preparing combined download: {e}")
